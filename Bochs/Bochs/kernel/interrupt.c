...
#define PIC_S_CTRL		0xa0				// 主片的控制端口是0xa0
...
static struct gate_desc idt[IDT_DESC_CNT];		//idt是中断描述符表
												//本质上就是个中断门描述符数数组
extern intr_handler intr_entry_table[IDT_DESC_CNT];		//声明引用定义在kernel.S
														//中的中断处理函数入口数组
char* intr_name[IDT_DESC_CNT];					//用于保存异常的名字
intr_handler idt_table[IDT_DESC_CNT];
//定义了中断处理程序数组，在kernel.S中定义的intrXXentry
...
static void pic_init(void) {
	...
	/* 初始化主片 */
	outb (PIC_M_CTRL, 0x11);		//ICW1: 边沿出发，级联8259，需要ICW4
...
static void exception_init(void) {
	...
	for (i = 0; i < IDT_DESC_CNT; i++) {
		...
		//默认为general_inte_handler，将来会由register_handler来注册具体处理函数
	...
	intr_name[12] = "#SS Stack Fault Excepion";
